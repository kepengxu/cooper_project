...
with tf.name_scope('layer_w_b'):
    with tf.name_scope('weights'):
        weights = {
            'wc1': tf.Variable(tf.random_normal([3, 3, 1, 32], stddev=stddev, name='wc1')),
            'wc2': tf.Variable(tf.random_normal([5, 5, 32, 64], stddev=stddev, name='wc2')),
            'wc3': tf.Variable(tf.random_normal([3, 3, 64, 128], stddev=stddev, name='wc3')),
            'wd1': tf.Variable(tf.random_normal([4 * 4 * 128, 1024], stddev=stddev, name='wd1')),
            'wd2': tf.Variable(tf.random_normal([1024, 10], stddev=stddev, name='wd2'))
        }
        tf.summary.histogram('layer1/weights', weights['wc1'])
        tf.summary.histogram('layer2/weights', weights['wc2'])
        tf.summary.histogram('layer3/weights', weights['wc3'])
    with tf.name_scope('biases'):
        biases = {
            'bd1': tf.Variable(tf.zeros([1024], name='bd1')),
            'bd2': tf.Variable(tf.zeros([n_classes], name='bd2'))
        }
        tf.summary.histogram('layer1/bias', biases['bd1'])
        tf.summary.histogram('layer2/bias', biases['bd2'])



def conv(_x, _w, _b, _keepratio, phase_train):
    with tf.name_scope('conv___layer1'):
        in_put_r = tf.reshape(_x, [-1, 28, 28, 1], name='reshape_input')  # [batch_sizes,height,weight,chanels]
        conv1 = tf.nn.conv2d(in_put_r, _w['wc1'], strides=[1, 1, 1, 1], padding="SAME",
                             name='conv1')  # strides=[batch_size_step,h_step,w_step,chanels_step]
        conv1bn = batch_norm_con(conv1, 32, phase_train=phase_train)
        conv1 = tf.nn.relu(conv1bn,name='relu')
        pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME",
                               name='max_pool_1')  # pool_window_size=[batch_size,h,w,chanels]
        # print pool1.size()
        pool_dr1 = tf.nn.dropout(pool1, _keepratio, name='dropout1')
    with tf.name_scope('conv___layer2'):
        ...
        conv2 = tf.nn.relu(conv2bn,name='relu')
    ...
    with tf.name_scope('conv___laye3r'):
        conv3 = tf.nn.conv2d(pool_dr2, _w['wc3'], strides=[1, 1, 1, 1], padding="SAME", name='conv3')
        conv3bn = batch_norm_con(conv3, 128, phase_train=phase_train)
        conv3 = tf.nn.relu(conv3bn,name='relu3')
    ...
    with tf.name_scope('full_connected_layer'):
        _dense1 = tf.reshape(pool_dr3, [-1, _w['wd1'].get_shape().as_list()[0]])
    ...
    for epoch in range(train_epochs):
        ...
        for i in range(total_batch):
            ...
            avg_cost += sess.run(cost,
                                 feed_dict={x: batch_xs, y: batch_ys, keepratio: 1., phase_train: False}) / total_batch
            if j%50==0:
                rs = sess.run(merged, feed_dict={x: batch_xs, y: batch_ys, keepratio: 1.0, phase_train: False})
